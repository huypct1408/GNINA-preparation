...
- **M**easurable: Delta AUC = AUC_Cancer_Cell_Line - AUC_HEK293 quantifies cancer selectivity
...
# Load the pre-normalized CCLE TPM matrix and extract Jurkat (or other cancer cells)+ HEK-293 samples.
...
if not cancer_found or not hek293_found:
    ...
    raise ValueError(f"Required cell lines not found in expression matrix!\n"
                     f"{L3B_CANCER_CELL_LINE} ({L3B_CANCER_MODEL_ID}): {cancer_found}\n"
...
print(f"  Cell lines: {L3B_CANCER_CELL_LINE} (cancer), HEK-293 (normal)")
print("="*64)
...
auc_cancer = auc_mtx.loc[L3B_CANCER_MODEL_ID]
auc_hek293 = auc_mtx.loc[L3B_NORMAL_MODEL_ID]

print(f"{L3B_CANCER_CELL_LINE} ({L3B_CANCER_MODEL_ID}) AUC scores:")
print(f"  Min: {auc_cancer.min():.4f}")
print(f"  Max: {auc_cancer.max():.4f}")
print(f"  Mean: {auc_cancer.mean():.4f}")
...
print(f"  {L3B_CANCER_CELL_LINE} mean AUC: {auc_cancer.mean():.4f}")
print(f"  HEK-293 mean AUC: {auc_hek293.mean():.4f}")
print("="*64)
## Stage 7: Calculate Delta AUC (Cancer Selectivity)

# **Delta AUC = AUC_Cancer_Cell_Line - AUC_HEK293**
...
# Calculate Delta AUC
delta_auc = AUC_CANCER - AUC_HEK293

# Create results DataFrame
results_df = pd.DataFrame({
    COL_REGULON_NAME: delta_auc.index,
    COL_AUC_CANCER: AUC_CANCER.values,
...
ax1.set_xlabel('Delta AUC (Cancer - HEK-293)', fontsize=12)
...
ax2.scatter(RESULTS_DF[COL_AUC_HEK293], RESULTS_DF[COL_AUC_CANCER], 
...
ax2.set_ylabel(f'AUC {L3B_CANCER_CELL_LINE} (Cancer)', fontsize=12)
ax2.set_title(f'AUC Comparison: {L3B_CANCER_CELL_LINE} vs HEK-293\n(Green = Selective)', fontsize=14)
ax2.legend()
...
if len(top30) > 0:
    ...
    ax.axvline(x=L3B_DELTA_AUC_THRESHOLD, color='red', linestyle='--', linewidth=2,
               label=f'Threshold ({L3B_DELTA_AUC_THRESHOLD})')
    ax.set_xlabel(f'Delta AUC ({L3B_CANCER_CELL_LINE} - HEK-293)', fontsize=12)
...
summary = {
    ...
    "cell_lines": {
        "cancer": {
            ...
            "mean_auc": float(AUC_CANCER.mean()),
        },
        "normal": {
    ...
    "l3a_aggregate": {
        ...
        "auc_cancer": float(l3a_results[COL_AUC_CANCER].values[0]),
...
print(f"  S - Compared regulon activity between {L3B_CANCER_CELL_LINE} (cancer) and HEK-293 (normal)") 
